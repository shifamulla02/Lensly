 ...
 * - Reads current page content as context, answers via Gemini API
 */
window.LenslyChatbot = (function () {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      let reply;
      if (apiKey && window.LenslySummarizer?.geminiChat) {
        reply = await window.LenslySummarizer.geminiChat(chatHistory, pageContext, apiKey);
      } else {
        reply = 'Please add your Gemini API key in the Lensly Settings tab to enable AI answers.';
      }
